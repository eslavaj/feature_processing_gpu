                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    cv::cuda::GpuMat gdescSource;
    gdescSource.upload(descSource);
    cv::cuda::GpuMat gdescRef;
    gdescRef.upload(descRef);
    ...
    if (matcherType.compare("MAT_BF") == 0)
    {
    	...
    	matcher = cv::cuda::DescriptorMatcher::createBFMatcher(normType);
    }
    /*
    ...
    { // nearest neighbor (best match)
    	...
    	matcher->match(gdescSource, gdescRef, matches); // Finds the best match for each descriptor in desc1
    ...
    { // k nearest neighbors (k=2)
    	...
    	double t = (double)cv::getTickCount();
    	matcher->knnMatch(gdescSource, gdescRef, knnMatches, 2);
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    cv::cuda::GpuMat gimg;
    gimg.upload(img);
    cv::cuda::GpuMat gdescriptors;
    gdescriptors.upload(descriptors);
	...
	case 3:
		/*ORB*/
		gextractor= cv::cuda::ORB::create();
		gextractor->compute(gimg, keypoints, gdescriptors);
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
	...
	cv::cuda::GpuMat gimg;
	gimg.upload(img);
	cv::Ptr<cv::cuda::FastFeatureDetector> gdetector;

	cv::cuda::GpuMat gkeypoints;
	...
	case 1:
		...
		gdetector = cv::cuda::FastFeatureDetector::create(threshold, true, cv::FastFeatureDetector::TYPE_9_16);
		gdetector->detect(gimg,keypoints);
		break;
	case 2:
		/*BRISK*/
